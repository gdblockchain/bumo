...
验证节点选举合约账户创建成功后，才可以进行后续的操作，且该账户是全局唯一的, 不能重复创建。
...
```
 ...
 在 dpos.js 文件指定的合约代码中, 以下变量可根据需要修改。但一旦验证节点选举账户创建成功，则不能再修改。
...
- 如果用户在已经支持了其他候选人的情况下发起投票支持操作，将视为更改支持的候选人。更改支持的候选人时，对应的票数会先从原候选人转移到新候选人；然后再执行增加/减少投票流程，即更新用户投票金额和对应候选人的得票数。

注意：减少投票的coin数额不得超过已有的投票金额。减少投票时转移coin应设置为0。

### 申请退出

- 验证节点候选人可通过此操作收回全部押金。退出流程分两步：
...
验证节点候选人可通过此操作收回部分押金，当候选人押金数额超过最低质押金额时，候选人可从质押金中收回一部分。
...
- 验证节点候选人审核，普通节点申请成为验证节点候选人时，由委员会成员审核其是否具备成为候选人的条件，包括物理节点配置，个人或者组织认证信息，信用程度等。
...
- 委员会成员可以向验证节点选举账户转移任意一笔资产或者一笔数额为 0 的 coin，提名新的委员。成为新得委员需获得委员会2/3以上的成员同意。
...
申请成功后可以通过[查询当前委员会信息](#查询当前委员会信息)，查询验证节点候选人信息。
...
- 所有用户均可向验证节点选举账户转移一笔coin投票支持某个KOL。
...
- 验证节点选举合约不仅支持选举功能，也支持选举配置的更新，选举配置更新分两个阶段，第一阶段由委员会对新的配置形成决议，第二阶段由验证节点集合对委员会配置更新决议进行确认。在有效期内确认完成后，将触发选举配置的更新，从下一区块开始采用新的配置。
...
- 委员会成员向验证节点选举账户转移任意一笔资产或者一笔数额为 0 的 coin。
- 候选人可以提议更新某一个参数，也可以同时更新多个参数，只需要在配置中填入需要更新的参数即可。
...
注意：只有验证节点候选人拥有投票权。若有效期内该配置更新提案未投票通过，则提案作废，选举配置保持不变。
...
任意一个用户账户可以通过向验证节点选举账户转移一笔 coin 作为押金，申请成为KOL。但能否成为KOL，是根据一定周期内获得的用户投票总票数决定的。
...
申请成功后可以通过[查询当前KOL信息](#查询当前KOL信息)，查询验证节点候选人信息。
