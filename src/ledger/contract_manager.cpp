...
//			args.GetReturnValue().Set(ret);
	...
	void V8Contract::CallBackSetValidatorCandidate(const v8::FunctionCallbackInfo<v8::Value>& args){
		...
		do{
			...
			if (v8_contract->GetParameter().this_address_ != GET_CONTRACT_VALIDATOR_ADDRESS){
	...
	void V8Contract::CallBackSetVoteForCandidate(const v8::FunctionCallbackInfo<v8::Value>& args){
		...
		do
		{
			...
			if (coin_amount > 0) {
				...
				candidate_new->set_coin_vote(new_coin_votes < 0 ? 0 : new_coin_votes); // set to 0 if new_coin_votes is negative
