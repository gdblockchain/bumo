...
namespace bumo {
	...
	public:
		...
		const static char *VALIDATOR_LEADER;
		...
		const static int32_t MAX_VALIDATORS = 4;
		const static int64_t MAX_ABNORMAL_RECORD = 5;
	...
	class SlowTimer : public utils::Singleton<bumo::SlowTimer>, public utils::Runnable {
	public:
		SlowTimer();
		~SlowTimer();
	...
	class Global : public utils::Singleton<bumo::Global>, public TimerNotify {
		asio::io_service io_service_;
