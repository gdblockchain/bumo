...
class ContractQuery : public testing::Test{
protected:
	void test();
	bumo::CTest ctest_;
};

TEST_F(ContractQuery, test){ test(); }

void ContractQuery::test(){
	...
	EXPECT_EQ(ctest_.Paycoin(param, contract_address_0), 0);
	EXPECT_NE(ctest_.Load("main"), "{\"result\":false}");
	EXPECT_NE(ctest_.Load("main"), "");
	EXPECT_EQ(ctest_.Load("init"), contract_address_0);
	...
	EXPECT_EQ(result["result"].asString(), contract_address_2);
}
